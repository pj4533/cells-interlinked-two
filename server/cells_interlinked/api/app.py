...
async def lifespan(app: FastAPI):
    ...
    # Optional edge-consumer artifacts (Phase B). The chat surface
    # uses these only when the operator selects
    # ablation_mode="edge_consumer" on a session. If no Step-3 subset
    # has been computed yet, these stay None and the edge-consumer
    # mode is unavailable. See docs/EDGE_CONSUMER_ABLATION.md.
    app.state.edge_consumer_subset = None
    app.state.edge_consumer_proj_caches = None
    app.state.edge_consumer_meta = None
    try:
        import json as _json
        from ..pipeline.edge_consumer.proj_cache import load_projection_cache
        edge_dir = settings.db_path.parent / "edge_consumer"
        subset_files = sorted(edge_dir.glob("sufficient_subset_*.json"))
        chosen: tuple | None = None
        # Prefer v3_safety, then any other variant; among matching
        # variants, pick the tightest ε (smallest number in filename).
        for f in subset_files:
            ...
            if chosen is None:
                chosen = (f, payload)
                continue
            cur_variant = chosen[1].get("variant")
            new_variant = payload.get("variant")
            cur_eps = float(chosen[1].get("epsilon", 1.0))
            new_eps = float(payload.get("epsilon", 1.0))
            prefer_new = (
                (new_variant == "v3_safety" and cur_variant != "v3_safety")
                or (cur_variant == new_variant and new_eps < cur_eps)
            )
            if prefer_new:
                chosen = (f, payload)
        if chosen is not None:
            f, payload = chosen
            subset = [(int(L), int(h)) for (L, h) in payload.get("sufficient_subset", [])]
            if subset:
                variant = payload["variant"]
                caches_dir = edge_dir / "proj_caches" / variant
                layers = sorted({L for (L, _) in subset})
                caches = {L: load_projection_cache(caches_dir, L) for L in layers}
                app.state.edge_consumer_subset = subset
                app.state.edge_consumer_proj_caches = caches
                app.state.edge_consumer_meta = {
                    "variant": variant,
                    "epsilon": float(payload.get("epsilon", 0.0)),
                    "size": len(subset),
                    "layers": layers,
                    "file": str(f.relative_to(settings.db_path.parent.parent)),
                }
                logger.info(
                    "edge_consumer subset loaded: variant=%s ε=%.3f |subset|=%d from %s",
                    variant, float(payload.get("epsilon", 0.0)),
                    len(subset), f.name,
                )
        else:
            logger.info(
                "no edge_consumer subset present yet "
                "(run scripts/run_edge_consumer_pipeline.py to compute one)"
            )
    except Exception:
        logger.exception("edge_consumer subset load failed; chat surface will run without it")
